...
void GrenadePrediction::Paint( )
{
	...
	if ( g::pLocalEntity->GetHealth( ) > 0 )
	{
		if ( ( type ) && path.size( ) > 1 )
		{
			...
			auto weapon = reinterpret_cast< C_BaseCombatWeapon* >( g_pEntityList->GetClientEntity( g::pLocalEntity->GetActiveWeaponIndex( ) ) );
			if ( !weapon ) return;
			bool in_attack = g::pCmd->buttons & IN_ATTACK, in_attack2 = g::pCmd->buttons & IN_ATTACK2;
			...
			for ( auto it = path.begin( ), end = path.end( ); it != end; ++it )
			{
				if ( ExplectedWorldToScrean( prev, nadeStart ) && ExplectedWorldToScrean( *it, nadeEnd ) )
					DrawLine( ( int )nadeStart.x, ( int )nadeStart.y, ( int )nadeEnd.x, ( int )nadeEnd.y, Color( g_Menu.Config.GrenadePredictionColor.red,
						g_Menu.Config.GrenadePredictionColor.green,
						g_Menu.Config.GrenadePredictionColor.blue ) );
				prev = *it;
			}
			for ( auto it = others.begin( ), end = others.end( ); it != end; ++it ) {
				...
				if ( ExplectedWorldToScrean( prevoth, nadeStart2 ) && ExplectedWorldToScrean( it->first, nadeEnd2 ) ) {
					//DrawLine( ( int )nadeStart2.x - 2, ( int )nadeStart2.y - 2, ( int )nadeStart2.x + 20, 1, Color( g_Menu.Config.GrenadePredictionColor.red,
					//	g_Menu.Config.GrenadePredictionColor.green,
					//	g_Menu.Config.GrenadePredictionColor.blue ) );
				}
